...
describe("conversations", () => {
  ...
  it("excludes another user's conversations from the list", async () => {
    ...
    const listForA =
      await database.listConversations(
        USER_A,
      );
...
describe("messages", () => {
  ...
  it("deletes from an anchor onward, for edit and regenerate", async () => {
    ...
    expect(
      await database.deleteMessagesFrom(
        conversation.id,
        USER_A,
        ids[1],
      ),
...
describe("usage and quotas", () => {
  it("counts only the user's own recent successful assistant responses", async () => {
    const conversationA =
      await database.createConversation({
        userId: USER_A,
        title: "A",
    ...
    /*
     * The quota is explicitly based on successful assistant responses.
     * Another user's generated replies must never consume USER_A's quota.
     */
    expect(
...
describe("attachments", () => {
  ...
  it("starts an attachment as pending, not ready", async () => {
    ...
    const attachment =
      await database.createAttachment({
        ...
        sizeBytes: 2048,
        storagePath:
          `${USER_A}/${conversation.id}/report.pdf`,
      });
  ...
  it("records the real storage path when the status advances", async () => {
    ...
    const attachment =
      await database.createAttachment({
        ...
        sizeBytes: 2048,
        storagePath:
          `pending/${USER_A}/placeholder`,
      });

    const finalPath =
      `${USER_A}/${conversation.id}/${attachment.id}-report.pdf`;
    ...
    const [stored] =
      await database.listAttachments(
        conversation.id,
        USER_A,
      );
  ...
  it("does not list another user's attachments", async () => {
    ...
    await database.createAttachment({
      ...
      sizeBytes: 100,
      storagePath:
        `${USER_A}/${conversation.id}/private.pdf`,
    });
...
describe("durability", () => {
  it("persists across adapter instances", async () => {
    ...
    const loaded =
      await reopened.getConversation(
        created.id,
        USER_A,
      );
