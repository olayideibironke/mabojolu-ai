 ...
 *
 *   Service role  Only usage rows, safety rows, attachment status transitions, and
 *                 admin aggregates. Each is an operation the user genuinely
 *                 cannot perform: a client that could write usage rows could forge
 *                 cost data, and one that could set an attachment to `ready` could
 *                 make the model treat an unvalidated file as readable.
 *
 * The `userId` parameters are not redundant with RLS. They keep ownership enforced
 * in application code too, so the two layers have to fail together for data to
 * leak.
 *
...
export class SupabaseDatabaseAdapter implements DatabaseAdapter {
  ...
  async upsertProfile(input: {
    ...
    if (error || !data) {
      throw new Error(`Could not upsert profile: ${error?.message ?? "unknown"}`);
    }
  ...
  async createConversation(
    ...
    if (error || !data) {
      throw new Error(
        `Could not create conversation: ${error?.message ?? "unknown"}`,
      );
    }
  ...
  async getConversation(
    ...
    const { data: messageRows } = await client
      .from("messages")
      .select("id, role, content, status, model_id, created_at")
    ...
    const feedbackByMessage = new Map(
      (feedbackRows ?? []).map((row) => [row.message_id, row.rating]),
    );
    ...
    return {
      ...
      messages: (messageRows ?? []).map((row) => ({
        ...
        ...(feedbackByMessage.has(row.id)
          ? { feedback: feedbackByMessage.get(row.id) as FeedbackRating }
          : {}),
      })),
    };
  }
  ...
  async listConversations(
    ...
    if (search) {
      ...
      matchingIds = [...new Set((hits ?? []).map((row) => row.conversation_id))];
    }
    ...
    if (search) {
      ...
      if (matchingIds && matchingIds.length > 0) {
        clauses.push(`id.in.(${matchingIds.join(",")})`);
      }
  ...
  async deleteConversation(
    ...
    // Soft delete, so an undo window exists. Every read filters `deleted_at is
    // null`, and a scheduled purge removes the rows permanently.
    const { error, count } = await client
      .from("conversations")
  ...
  async appendMessage(input: AppendMessageInput): Promise<ChatMessage> {
    ...
    if (error || !data) {
      throw new Error(
        `Could not append message: ${error?.message ?? "unknown"}`,
      );
    }
    ...
    return {
      ...
      createdAt: data.created_at,
      ...(data.model_id ? { model: data.model_id } : {}),
    };
  }
      ...
      .update(
        {
          ...
          ...(patch.usage === undefined
            ...
            : {
                ...
                cache_read_tokens: patch.usage.cacheReadTokens ?? null,
                cache_write_tokens: patch.usage.cacheWriteTokens ?? null,
              }),
          // Explicit null clears a previous error when a retry succeeds, which
          // the schema's status/error_code constraint requires.
  ...
  async recordUsage(input: UsageEventInput): Promise<void> {
    ...
    if (error) {
      ...
      console.error("[mabojolu] usage write failed", { code: error.code });
    }
  }
  ...
  async recordSafetyEvent(input: SafetyEventInput): Promise<void> {
    ...
    if (error) {
      console.error("[mabojolu] safety write failed", { code: error.code });
    }
  }
  ...
  async countRecentMessages(userId: string, sinceIso: string): Promise<number> {
    ...
    if (error) {
      ...
      console.error("[mabojolu] quota count failed", { code: error.code });
  ...
  async createAttachment(input: AttachmentInput): Promise<AttachmentRecord> {
    ...
    const { data, error } = await client
      ...
      .select(
        "id, conversation_id, filename, mime_type, size_bytes, storage_path, status, failure_reason, created_at",
      )
      .single();

    if (error || !data) {
      throw new Error(
        `Could not create attachment: ${error?.message ?? "unknown"}`,
      );
    }
  ...
  async updateAttachmentStatus(
     ...
     * because a client that could set status to 'ready' could make the model treat
     * an unvalidated file as readable. Only server-side processing advances this.
     */
    const client = this.serviceClient();
    ...
    const { error, count } = await client
      ...
      .eq("id", attachmentId)
      // Ownership is still checked, even though the service role would not
      // require it.
      .eq("user_id", userId);
  ...
  async deleteAttachment(
    ...
    if (storageError) {
      // The row is already gone, so report success; the orphan sweeper will
      // collect the file.
      console.error("[mabojolu] storage delete failed", {
  ...
  async getAdminMetrics(): Promise<AdminMetrics> {
    ...
    const [users, conversations, messages, usage, feedback, errors, safety] =
      await Promise.all([
        ...
        client
          ...
          .limit(10_000),
        client.from("message_feedback").select("rating").limit(10_000),
        client
          .from("messages")
          .select("error_code")
          .not("error_code", "is", null)
          .limit(1_000),
        client.from("safety_events").select("kind, severity").limit(5_000),
      ]);
    ...
    const safetyCounts = new Map<string, AdminMetrics["safetyEvents"][number]>();
    for (const row of safety.data ?? []) {
    ...
    return {
      ...
      feedback: {
        up: (feedback.data ?? []).filter((row) => row.rating === "up").length,
        down: (feedback.data ?? []).filter((row) => row.rating === "down").length,
      },
      recentErrors: [...errorCounts.entries()]
        .map(([code, count]) => ({ code, count }))
        .sort((a, b) => b.count - a.count),
      safetyEvents: [...safetyCounts.values()].sort((a, b) => b.count - a.count),
    };
  }
  ...
  async deleteAllUserData(userId: string): Promise<void> {
     ...
     * driven by per-table policies could leave rows behind, which would break the
     * promise made in the privacy documentation.
     *
     * Cascades from auth.users handle most of this, but attachments in storage
     * are removed explicitly first: no foreign key reaches into a bucket.
     */
    const client = this.serviceClient();
    ...
    await client.from("attachments").delete().eq("user_id", userId);
    await client.from("message_feedback").delete().eq("user_id", userId);
    ...
    // de-identified. That keeps aggregate cost history intact without retaining
    // personal data.
  }
}
...
function toAttachment(row: AttachmentRow): AttachmentRecord {
  return {
    ...
    status: row.status as AttachmentRecord["status"],
